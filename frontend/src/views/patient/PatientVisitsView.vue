<template>
  <PageContainer title="我的就诊" desc="查看个人历史就诊记录、医嘱和处方信息">
    <el-collapse>
      <el-collapse-item v-for="item in list" :key="item.id" :title="`${item.visitDate} ｜ ${item.doctorName} ｜ ${item.diagnosis || '未填写诊断'}`" :name="item.id">
        ...
        <div class="page-card page-block">
          ...
          <div v-html="item.adviceHtml || '<p>暂无医嘱内容</p>'" class="rich-content" style="min-height:auto;border:1px solid #e5ebf4;border-radius:8px;"></div>
        </div>
      </el-collapse-item>
    </el-collapse>
  </PageContainer>
