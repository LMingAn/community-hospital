...
async function submitChange() { await authApi.patientPassword(changeForm); successTip('密码修改成功'); changeForm.oldPassword=''; changeForm.newPassword='' }
async function getCode() { const res = await authApi.patientResetCode({ phone: resetForm.phone }); demoCode.value = res.data?.code || ''; successTip('验证码已生成') }
async function submitReset() { await authApi.patientResetPassword(resetForm); successTip('密码重置成功'); resetForm.phone=''; resetForm.code=''; resetForm.newPassword=''; demoCode.value='' }
</script>
