...
const form = reactive({ username:'', name:'', gender:'男', age:18, phone:'', idCard:'', createdAt:'' })
async function load(){ 
  const res = await authApi.patientProfile(); 
  Object.assign(form, res.data || {}) 
}
async function submit(){ 
  await authApi.updatePatientProfile({ name: form.name, gender: form.gender, age: form.age, phone: form.phone, idCard: form.idCard }); 
  successTip('个人信息已更新'); 
  load() 
}
onMounted(load)
