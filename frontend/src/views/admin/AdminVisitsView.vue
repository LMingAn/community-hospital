  ...
  <PageContainer title="患者就诊" desc="支持就诊信息的新增、查询、编辑与删除">
    ...
    <el-dialog v-model="visible" :title="form.id ? '编辑就诊记录' : '新增就诊记录'" width="720px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="88px">
        <div class="form-grid">
          <el-form-item label="挂号单号" prop="appointmentId" required><el-select v-model="form.appointmentId" @change="syncAppointment"><el-option v-for="item in appointments" :key="item.id" :label="item.appointmentNo" :value="item.id" /></el-select></el-form-item>
          ...
          <el-form-item label="医生" prop="doctorId" required><el-select v-model="form.doctorId"><el-option v-for="item in doctors" :key="item.id" :label="item.name" :value="item.id" /></el-select></el-form-item>
...
const rules = {
  appointmentId: [{ required: true, message: '请选择挂号单号', trigger: 'change' }],
...
function openEdit(row){ Object.assign(form, row, { needHospitalization: !!row.needHospitalization }); visible.value = true; resetValidation() }
