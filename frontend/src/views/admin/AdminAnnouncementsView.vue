  ...
  <PageContainer title="系统公告" desc="管理员可对系统公告进行新增、编辑、删除和查询">
    ...
    <el-table :data="filteredList" border>
      ...
      <el-table-column label="状态" width="100">
        <template #default="{ row }"><span class="status-tag" :class="row.status === 1 ? 'status-success' : 'status-warning'">{{ row.status === 1 ? '已发布' : '停用' }}</span></template>
      </el-table-column>
...
async function remove(row){ 
  await confirmAction(`确认删除公告“${row.title}”吗？`); 
