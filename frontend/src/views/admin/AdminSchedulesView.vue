      ...
      <el-form :model="form" label-width="88px">
        <div class="form-grid">
          ...
          <el-form-item label="最大号源">
            <el-input-number v-model="form.maxNumber" :min="1" :max="100" />
          </el-form-item>
          <el-form-item label="挂号费用">
            <el-input-number v-model="form.fee" :min="1" :max="500" />
          </el-form-item>
...
import { successTip, weekdayLabel, confirmAction } from '../../utils'
const list = ref([]); 
const doctors = ref([]); 
const visible = ref(false); 
const form = reactive({ id:'', doctorId: '', weekday: 1, period: '上午', maxNumber: 20, fee: 15, status:1 })
...
async function loadAll() { 
  const [scheduleRes, doctorRes] = await Promise.all([adminApi.schedules(), adminApi.doctors()]); 
  list.value = scheduleRes.data || []; doctors.value = doctorRes.data || [] 
}
function openCreate(){ reset(); visible.value = true }
function openEdit(row){ Object.assign(form, row); visible.value = true }
async function submit() { 
  if(form.id) 
    await adminApi.updateSchedule(form.id, form); 
  else 
...
async function remove(row){ 
  await confirmAction(`确认删除 ${row.doctorName} 的排班吗？`); 
