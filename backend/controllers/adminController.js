...
exports.stats = async (req, res, next) => {
  try {
    ...
    const [todayDoctors] = await pool.query(`SELECT d.name AS doctorName, dp.name AS departmentName, w.period, w.max_number AS maxNumber FROM weekly_schedules w JOIN doctors d ON w.doctor_id = d.id JOIN departments dp ON d.department_id = dp.id WHERE w.weekday = WEEKDAY(CURDATE()) + 1 AND w.status = 1 ORDER BY dp.id, d.id, FIELD(w.period, '上午','下午','夜间')`);
    const [announcements] = await pool.query(`SELECT id, title, content, DATE_FORMAT(publish_time, '%Y-%m-%d %H:%i:%s') AS publishTime, status FROM announcements ORDER BY publish_time DESC LIMIT 5`);
...
exports.listDoctors = async (req, res, next) => {
  try {
    const [rows] = await pool.query(`SELECT d.id, d.department_id AS departmentId, d.username, d.name, d.gender, d.title, d.specialty, d.phone, d.intro, d.status, dp.name AS departmentName FROM doctors d JOIN departments dp ON d.department_id = dp.id ORDER BY d.id`);
    res.json({ success: true, data: rows });
  } catch (error) { next(error); }
};
exports.createDoctor = async (req, res, next) => {
  try {
    const { departmentId, username, password, name, gender, title, specialty, phone, intro, status } = req.body;
    if (!departmentId || !username || !password || !name || !title || !phone) return res.status(400).json({ success: false, message: '请完整填写医生信息' });
    await pool.query(`INSERT INTO doctors (department_id, username, password_hash, name, gender, title, specialty, phone, intro, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [departmentId, username, hashPassword(password), name, gender || '男', title, specialty || '', phone, intro || '', status ?? 1]);
    res.json({ success: true, message: '医生新增成功' });
  } catch (error) { next(error); }
};
exports.updateDoctor = async (req, res, next) => {
  try {
    const { departmentId, username, name, gender, title, specialty, phone, intro, status, password } = req.body;
    await pool.query(`UPDATE doctors SET department_id = ?, username = ?, name = ?, gender = ?, title = ?, specialty = ?, phone = ?, intro = ?, status = ? WHERE id = ?`, [departmentId, username, name, gender || '男', title, specialty || '', phone, intro || '', status ?? 1, req.params.id]);
...
exports.listPatients = async (req, res, next) => {
  try {
    const [rows] = await pool.query(`SELECT id, username, name, gender, age, phone, id_card AS idCard, balance, status, DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:%s') AS createdAt FROM patients ORDER BY id DESC`);
    res.json({ success: true, data: rows });
  } catch (error) { next(error); }
};
exports.createPatient = async (req, res, next) => {
  try {
    ...
    await pool.query(`INSERT INTO patients (username, password_hash, name, gender, age, phone, id_card, balance, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [username, hashPassword(password), name, gender || '男', age || null, phone, idCard || '', balance || 0, status ?? 1]);
...
exports.updatePatient = async (req, res, next) => {
  try {
    ...
    await pool.query(`UPDATE patients SET username = ?, name = ?, gender = ?, age = ?, phone = ?, id_card = ?, balance = ?, status = ? WHERE id = ?`, [username, name, gender || '男', age || null, phone, idCard || '', balance || 0, status ?? 1, req.params.id]);
...
exports.listAnnouncements = async (req, res, next) => { try { const [rows] = await pool.query(`SELECT id, title, content, DATE_FORMAT(publish_time, '%Y-%m-%d %H:%i:%s') AS publishTime, status FROM announcements ORDER BY publish_time DESC`); res.json({ success: true, data: rows }); } catch (error) { next(error); } };
...
exports.listSchedules = async (req, res, next) => {
  try {
    const [rows] = await pool.query(`SELECT w.id, w.doctor_id AS doctorId, w.weekday, w.period, w.max_number AS maxNumber, w.fee, w.status, d.name AS doctorName, dp.name AS departmentName FROM weekly_schedules w JOIN doctors d ON w.doctor_id = d.id JOIN departments dp ON d.department_id = dp.id ORDER BY w.weekday, d.id, FIELD(w.period, '上午','下午','夜间')`);
    res.json({ success: true, data: rows });
  } catch (error) { next(error); }
};
exports.createSchedule = async (req, res, next) => { try { const { doctorId, weekday, period, maxNumber, fee, status } = req.body; if (!doctorId || !weekday || !period) return res.status(400).json({ success: false, message: '请完整填写排班信息' }); await pool.query(`INSERT INTO weekly_schedules (doctor_id, weekday, period, max_number, fee, status) VALUES (?, ?, ?, ?, ?, ?)`, [doctorId, weekday, period, maxNumber || 20, fee || 15, status ?? 1]); res.json({ success: true, message: '排班新增成功' }); } catch (error) { next(error); } };
exports.updateSchedule = async (req, res, next) => { try { const { doctorId, weekday, period, maxNumber, fee, status } = req.body; await pool.query(`UPDATE weekly_schedules SET doctor_id = ?, weekday = ?, period = ?, max_number = ?, fee = ?, status = ? WHERE id = ?`, [doctorId, weekday, period, maxNumber || 20, fee || 15, status ?? 1, req.params.id]); res.json({ success: true, message: '排班更新成功' }); } catch (error) { next(error); } };
...
exports.listAppointments = async (req, res, next) => {
  try {
    const [rows] = await pool.query(`SELECT a.id, a.appointment_no AS appointmentNo, a.patient_id AS patientId, a.doctor_id AS doctorId, a.department_id AS departmentId, DATE_FORMAT(a.visit_date, '%Y-%m-%d') AS visitDate, a.period, a.queue_no AS queueNo, a.fee, a.symptom, a.triage_result AS triageResult, a.status, p.name AS patientName, p.phone, d.name AS doctorName, dp.name AS departmentName FROM appointments a JOIN patients p ON a.patient_id = p.id JOIN doctors d ON a.doctor_id = d.id JOIN departments dp ON a.department_id = dp.id ORDER BY a.visit_date DESC, FIELD(a.period, '上午','下午','夜间'), a.queue_no`);
    res.json({ success: true, data: rows });
  } catch (error) { next(error); }
};
exports.createAppointment = async (req, res, next) => {
  try {
    ...
    await pool.query(`INSERT INTO appointments (appointment_no, patient_id, doctor_id, department_id, visit_date, period, queue_no, fee, symptom, triage_result, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [appointmentNo, patientId, doctorId, departmentId, visitDate, period, queueNo || 1, fee || 0, symptom || '', triageResult || '', status || '待叫号']);
...
exports.updateAppointment = async (req, res, next) => {
  try {
    const { appointmentNo, patientId, doctorId, departmentId, visitDate, period, queueNo, fee, symptom, triageResult, status } = req.body;
    await pool.query(`UPDATE appointments SET appointment_no = ?, patient_id = ?, doctor_id = ?, department_id = ?, visit_date = ?, period = ?, queue_no = ?, fee = ?, symptom = ?, triage_result = ?, status = ? WHERE id = ?`, [appointmentNo, patientId, doctorId, departmentId, visitDate, period, queueNo || 1, fee || 0, symptom || '', triageResult || '', status || '待叫号', req.params.id]);
...
exports.createVisit = async (req, res, next) => {
  try {
    ...
    await pool.query(`INSERT INTO visit_records (appointment_id, patient_id, doctor_id, diagnosis, advice_html, prescription, need_hospitalization) VALUES (?, ?, ?, ?, ?, ?, ?)`, [appointmentId, patientId, doctorId, diagnosis || '', adviceHtml || '', prescription || '', needHospitalization ? 1 : 0]);
...
exports.updateVisit = async (req, res, next) => {
  try {
    ...
    const { appointmentId, patientId, doctorId, diagnosis, adviceHtml, prescription, needHospitalization } = req.body;
    await pool.query(`UPDATE visit_records SET appointment_id = ?, patient_id = ?, doctor_id = ?, diagnosis = ?, advice_html = ?, prescription = ?, need_hospitalization = ? WHERE id = ?`, [appointmentId || current.appointment_id, patientId || current.patient_id, doctorId || current.doctor_id, diagnosis || '', adviceHtml || '', prescription || '', needHospitalization ? 1 : 0, req.params.id]);
...
exports.createHospitalization = async (req, res, next) => {
  try {
    ...
    await pool.query(`INSERT INTO hospitalization_records (patient_id, visit_record_id, ward_no, bed_no, reason_text, status) VALUES (?, ?, ?, ?, ?, ?)`, [patientId, visitRecordId, wardNo || '', bedNo || '', reasonText || '', status || '待入院']);
...
exports.updateHospitalization = async (req, res, next) => {
  try {
    const { patientId, visitRecordId, wardNo, bedNo, reasonText, status } = req.body;
    await pool.query(`UPDATE hospitalization_records SET patient_id = ?, visit_record_id = ?, ward_no = ?, bed_no = ?, reason_text = ?, status = ? WHERE id = ?`, [patientId, visitRecordId, wardNo || '', bedNo || '', reasonText || '', status || '待入院', req.params.id]);
